...
- **Blazor page**: `Pages/Admin.razor` injects `AdminMetricsService`, refreshes data during `OnInitializedAsync`, and renders the Activation Explorer, Client Connections table, storage tiers, ingest summary, and Graph RDF Import card. The hierarchy view uses MudBlazor `MudTreeView` with node details in a side panel. Helpers such as `FormatMemoryUsage` and `GetGraphStatusLabel` keep the UI readable.
...
3. **Manual graph import** 窶・when the operator clicks 窶懊う繝ｳ繝昴・繝医ｒ螳溯｡娯・ the page builds a `GraphSeedRequest` (path + tenant) and calls `AdminMetricsService.TriggerGraphSeedAsync`, which relays the call to `GraphSeedGrain.SeedAsync`. That grain invokes `GraphSeeder`, which parses RDF, writes grains, and returns success metadata. The UI rerenders the status card with the result and optionally shows feedback text.
