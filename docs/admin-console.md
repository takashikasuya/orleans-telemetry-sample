...
- **Telemetry monitoring panel (top)**: telemetry monitoring is shown near the top (below overview), keeps multi-point trend comparison, restores optional SignalR realtime updates for the selected point, and adds a toggleable ingest volume summary (last 1/5 minutes) derived from API request logs.
- **API request log monitor**: Admin UI can search recent ApiGateway request logs (tenant/path/status filters) and auto-refresh every 5 seconds for near-real-time operations monitoring.
